...
// TODO let's persist this to disk in the .config folder, loading it during startup (or compute)
//      and then update it as new items are discovered
static ICNS_CACHE: Lazy<RwLock<HashMap<String, String>>> =
...
// TODO this makes all the assumptions about macos. You need to define platform specific
//      startegies with macro guards.
pub fn to_icns(p: &Path) -> Result<String, anyhow::Error> {
  let pathkey = p.to_str().unwrap().to_string();
  if let Some(v) = ICNS_CACHE.read().get(&pathkey) {
    return Ok(v.clone());
  }
  let new_v = ICNS_CACHE
    .write()
    .entry(pathkey)
    .or_insert({
